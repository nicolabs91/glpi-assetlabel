(() => {
  ...
  const initializeLivePreview = () => {
    ...
    const update = () => {
      ...
      const rotationValue = Number(rotation.value) || 0;
      const rotated = rotationValue === 90 || rotationValue === 270;
      const pageWidth = labelWidth;
      const pageHeight = labelHeight;
      const printLabelWidth = rotated ? labelHeight : labelWidth;
      const printLabelHeight = rotated ? labelWidth : labelHeight;
      ...
      document.documentElement.style.setProperty('--assetlabel-page-width', `${pageWidth}mm`);
      document.documentElement.style.setProperty('--assetlabel-page-height', `${pageHeight}mm`);
      document.documentElement.style.setProperty(
        '--assetlabel-print-label-width',
        `${printLabelWidth}mm`,
      );
      document.documentElement.style.setProperty(
        '--assetlabel-print-label-height',
        `${printLabelHeight}mm`,
      );
      pageSize.textContent = `:root{--assetlabel-width:${labelWidth}mm;`
        + `--assetlabel-height:${labelHeight}mm;`
        + `--assetlabel-page-width:${pageWidth}mm;`
        + `--assetlabel-page-height:${pageHeight}mm;`
        + `--assetlabel-print-label-width:${printLabelWidth}mm;`
        + `--assetlabel-print-label-height:${printLabelHeight}mm}`
        + (usesNativePrintPaper
          ? '@media print{@page{margin:0}}'
          : `@media print{@page{size:${pageWidth}mm ${pageHeight}mm;margin:0}}`);
